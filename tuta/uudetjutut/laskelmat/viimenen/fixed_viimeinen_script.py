...
Tuotannon aloittaminen vaatisi 310000 € investoinnin tuotantolaitteistoihin. Markkinatutkimuksen perusteella Biohajotus Oy:n johto odottaa EkopoltePlussan myyntimäärän olevan 130000 litraa ensimmäisenä vuotena, ja kysynnän vuosittainen kasvu saavuttaisi seuraavina tilikausina jopa 4.6 %. EkopoltePlussan suuremman energiatiheyden johdosta myös keskimääräisen myyntihinnan arvioidaan olevan huomattavasti alkuperäistä Ekopoltetta korkeammalla, noin 4.18 €/litra. Tuotantoprosessin suuremman vety- ja fosforikulutuksen johdosta kuitenkin ravinneannosten kokoa on kasvatettava siten, että myös tuotteen raaka-ainekustannukset nousisivat tasolle 1.17 €/tuotettu litra. Yksinomaan EkopoltePlus-tuotantoon liittyvien muiden kustannusten odotetaan olevan noin 160000 €/vuosi. 

Pääoman keskimääräinen tuottovaatimus on Biohajotus Oy:lle investointipäätöksen tekohetkellä 9.9 %. Käyttöpääoman sitoutumista ei tarvitse ottaa huomioon. Tuotantolaitteiston eliniäksi arvioidaan noin viisi vuotta, ja se poistetaan tänä aikana tasapoistoin, jonka jälkeen voidaan olettaa, että tuotantolaitteisto on käyttökelvoton ja arvoton. Hinnoissa ei tarvitse huomioida arvonlisäveroa, yhteisövero on 20 %. 
...
Investoinnin myötä alkanut EkopoltePlus-tuotanto on nyt ollut käynnissä kaksi vuotta, ja investoinnin kolmas tilikausi on juuri alkamassa. Myyntimäärät vastasivat odotuksia vuosina 1 ja 2, mutta markkinat ovat kääntyneet laskusuhdanteeseen. Vuosille 3-5 ennustetaan huomattavasti heikompaa, 1.7 % kasvua. Muutokset yrityksen rahoitusrakenteessa ovat myös vaikuttaneet tuottovaateisiin siten, että pääoman keskimääräinen tuottovaade on 12.1 %.

Eräs energiateollisuusalan johtava korporaatio on jo pitkään ollut kiinnostunut Biohajotus Oy:n teknologiasta, ja on jopa harkinnut yrityksen ostamista. Korporaatio haluaa kuitenkin ensin testata Biohajotus Oy:n uutta tekniikkaa tuotannon skaalautuvuuden varmistamiseksi, ja tarjoutuu ostamaan EkopoltePlus-tuotantolaitteiston ensi vuoden alussa, käypään 186000 € hintaan (mikä sattumalta vastaa sen tämänhetkistä arvoa taseessa, eli myynnillä ei olisi verovaikutusta). Kauppojen ehtona kuitenkin on, että maksu suoritetaan vasta tilikauden 3 lopussa kun laitteisto on integroitu jalostamoon ja sen käyttö alkaa.
...
tuotannon_aloittamisen_kustannus = 310000 # euros
myyntimäärä_litraa = 130000 # liters
vuosittainen_kasvu = 0.046 # 5.7 percent
myyntihinta_per_litra = 4.18 # euros / litre
raaka_ainekustannukset = 1.17 # euros / produced litre
muut_kustannukset = 160000 # euros / year
tuottovaatimus = 0.099 # 9.5 %

tuotantolaitteiston_elinikä = 5 # 5 years
yhteisövero = 0.2 # 20 %
uusi_tuottovaade = 0.121 # Uusi tuottovaade kysymyksessä 12
uusi_kasvuprosentti = 0.017 # Uusi kasvuprosentti kysymyksessä 12
tuotantolaitteiston_myyntihinta_tehtävässä_12 = 186000 # Tuotantolaitteiston myyntihinta tehtävässä 12
...
def toinen_sivu():
	...
	for t in range(1, tuotantolaitteiston_elinikä + 1):
		...
		käyttökate_loop = liikevaihto_loop - materiaalikustannukset - muut_kustannukset

		# Liikevoitto = käyttökate - poistot
		liikevoitto_loop = käyttökate_loop - vuotuiset_poistot

		# Verot = liikevoitto * vero
		verot_loop = liikevoitto_loop * yhteisövero

		# Vapaat kassavirrat FCF = liikevoitto*(1 - vero) + poistot
		FCF_t_loop = liikevoitto_loop * (1 - yhteisövero) + vuotuiset_poistot
